      ...
      if (advertisedDevice.haveServiceData())
      {
        ...
        for (int j = 0; j < serviceDataCount; j++)
        {
          ...
          { // process only the devices close enough
            JsonObject &BLEdataOut = process_bledata(mac, BLEdata);
      ...
      while ((pos = returnedString.lastIndexOf(BLEdelimiter)) != -1)
      {
        ...
        { // we extract data only if we have infos (BLEdelimiter length + 4)
          ...
          if ((strlen(d[0].extract)) == 12) // if a BLE device is detected we analyse it
          {
            ...
            if (abs((int)BLEdata["rssi"] | 0) < abs(Minrssi))
            { // process only the devices close enough
              JsonObject &BLEdataOut = process_bledata(d[0].extract, BLEdata);
              #if !pubKnownBLEServiceData
              if(BLEdataOut.containsKey("model") && BLEdataOut.containsKey("servicedata")) // if we have recognised a sensor we remove the servicedata key/value
              {
                Log.trace(F("Removing service data" CR));
...
JsonObject& process_bledata(char * mac,  JsonObject &BLEdata){
  Log.trace(F("Checking BLE service data validity" CR));
  if(valid_service_data(BLEdata["servicedata"].as<const char*>()))
  {
    Log.trace(F("Searching BLE device data" CR));
    BLEdevice *device = getDeviceByMac(mac);
    String service_data(BLEdata["servicedata"].as<const char*>());
    Log.trace(F("Is it a mi flora ?" CR));
    int pos = -1;
    pos = strpos(BLEdata["servicedata"].as<const char*>(), "209800");
    if (pos != -1)
    {
      Log.trace(F("mi flora data reading at pos: %d" CR), pos);

      BLEdata.set("model", "HHCCJCY01HHCC");
      service_data = service_data.substring(pos - 2);
      BLEdata.set("servicedata", (char *)service_data.c_str());
      if (!isDiscovered(device))
        MiFloraDiscovery(mac);

      return process_sensors(2, BLEdata, mac);
    }
    Log.trace(F("Is it a vegtrug ?" CR));
    pos = strpos(BLEdata["servicedata"].as<const char*>(), "20bc03");
    if (pos != -1 && service_data.length() > ServicedataMinLength)
    {
      Log.trace(F("vegtrug data reading at pos: %d" CR), pos);
    ...
    pos = strpos(BLEdata["servicedata"].as<const char*>(), "20aa01");
    if (pos != -1 && service_data.length() > ServicedataMinLength)
    {
      Log.trace(F("LYWSDCGQ data reading at pos: %d" CR), pos);
      ...
      return process_sensors(0, BLEdata, mac);
    }
    Log.trace(F("Is it a LYWSD02?" CR));
    pos = strpos(BLEdata["servicedata"].as<const char*>(), "205b04");
    if (pos != -1 && service_data.length() > ServicedataMinLength)
    {
      Log.trace(F("LYWSD02 data reading at pos: %d" CR), pos);
    ...
    pos = strpos(BLEdata["servicedata"].as<const char*>(), "304703");
    if (pos != -1 && service_data.length() > ServicedataMinLength)
    {
    ...
    pos = strpos(BLEdata["servicedata"].as<const char*>(), "4030dd");
    if (pos != -1)
    {
      Log.trace(F("MUE4094RT data reading at pos: %d" CR), pos);

      BLEdata.set("model", "MUE4094RT");
      service_data = service_data.substring(pos);
      BLEdata.set("servicedata", (char *)service_data.c_str());
      if (!isDiscovered(device))
        MiLampDiscovery(mac);

      return process_milamp(BLEdata, mac);
    }
    Log.trace(F("Is it a CGP1W?" CR));
    pos = strpos(BLEdata["servicedata"].as<const char*>(), "08094c");
    if (pos != -1 && service_data.length() > ServicedataMinLength)
    {
      Log.trace(F("CGP1W data reading at pos: %d" CR), pos);
      ...
      return process_cleargrass(BLEdata, mac, true);
    }
    Log.trace(F("Is it a CGG1" CR));
    pos = strpos(BLEdata["servicedata"].as<const char*>(), "080774");
    if (pos != -1)
    {
      ...
      service_data = service_data.substring(pos);
      BLEdata.set("servicedata", (char *)service_data.c_str());
      return process_cleargrass(BLEdata, mac, false);
    }
    Log.trace(F("Is it a CGD1?" CR));
    pos = strpos(BLEdata["servicedata"].as<const char*>(), "080caf");
    if (pos == -1 && service_data.length() > ServicedataMinLength)
      pos = strpos(BLEdata["servicedata"].as<const char*>(), "080c09");
    if (pos != -1 && service_data.length() > ServicedataMinLength)
    {
      Log.trace(F("CGD1 data reading at pos: %d" CR), pos);
      service_data = service_data.substring(pos);
      BLEdata.set("servicedata", (char *)service_data.c_str());
    ...
    if(BLEdata.containsKey("servicedatauuid"))
    {
      ...
      if (strstr(BLEdata["servicedatauuid"], "fee0") != NULL)
      {
        ...
        return process_miband(BLEdata, mac);
      }
      Log.trace(F("Is it a XMTZC04HM?" CR));
      if (strstr(BLEdata["servicedatauuid"], "181d") != NULL)
      {
        ...
        return process_scale_v1(BLEdata, mac);
      }
      Log.trace(F("Is it a XMTZC05HM?" CR));
      if (strstr(BLEdata["servicedatauuid"], "181b") != NULL)
      {
        ...
        return process_scale_v2(BLEdata, mac);
      }
    }
  }
  else
...
JsonObject& process_sensors(int offset, JsonObject &BLEdata, char *mac_adress)
{
...
JsonObject& process_scale_v1(JsonObject &BLEdata, char *mac_adress)
{
  const char * servicedata = BLEdata["servicedata"].as<const char *>();
...
JsonObject& process_scale_v2(JsonObject &BLEdata, char *mac_adress)
{
  const char * servicedata = BLEdata["servicedata"].as<const char *>();
...
JsonObject& process_miband(JsonObject &BLEdata, char *mac_adress)
{
  const char * servicedata = BLEdata["servicedata"].as<const char *>();
...
JsonObject& process_milamp(JsonObject &BLEdata, char *mac_adress)
{
  const char * servicedata = BLEdata["servicedata"].as<const char *>();
...
JsonObject& process_cleargrass(JsonObject &BLEdata, char *mac_adress, boolean air)
{
  const char * servicedata = BLEdata["servicedata"].as<const char *>();
